...
class LocationService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ...
            ACTION_FIND_NEARBY -> {
                Log.d("LocationService", "Finding nearby events")
    ...
    private fun startLocationUpdates(nearby: Boolean = false) {
        locationClient.getLocationUpdates(10000L)
            ...
            .onEach { location ->
                ...
                if (nearby) {
                    checkProximityToEvents(location.latitude, location.longitude)
                }
    ...
    private fun checkProximityToEvents(latitude: Double, longitude: Double) {
            ...
            .addOnSuccessListener { result ->
                for (document in result) {
                    ...
                    geoPoint?.let {
                        ...
                        if (distance <= 100 && !notifiedEvents.contains(document.id)) {
                            val eventName = document.getString("eventName") ?: "Event"
                            Log.d("LocationService", "Retrieved eventName: $eventName")
                            sendNearbyEventNotification(document.id, eventName)
                            notifiedEvents.add(document.id)
                        }
                    }
                }
            }
            .addOnFailureListener { e ->
                Log.e("LocationService", "Error fetching events", e)
            }
    }
    ...
    private fun sendNearbyEventNotification(eventId: String, eventName: String) {
        ...
        val notification = NotificationCompat.Builder(this, notificationChannelId)
            ...
            .setContentText("You are near $eventName!")
            .setSmallIcon(R.drawable.ic_launcher_background)
            .setContentIntent(pendingIntent)
            .build()

        val notificationManager = getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
        notificationManager.notify(NEARBY_EVENT_NOTIFICATION_ID, notification)
    }
    ...
    private fun createNotificationChannel() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(
                ...
                description = "Notifies when you are near a event."
            }
    ...
    companion object {
        ...
        private const val NEARBY_EVENT_NOTIFICATION_ID = 2
    }
}